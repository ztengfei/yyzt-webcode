...
export default function TeansferTable(props: any) {
    ...
    const renderCell = useCallback((user: any, columnKey: any) => {
        ...
        switch (columnKey) {
            ...
            case "control":
                if (user.zxStatus == 3) {
                    // 转写完成 transfer/complete?
                    return (
                        <div className="relative flex items-center gap-3 w-[140px]">
                            {/* user.zxFiles[0].id */}
                            <Link
                                href={`/transfer/complete?order=${user.id}`}
                                underline="none"
                                className="text-sm"
                            >
                                详情
                ...
                if (user.zxStatus == 4) {
                    ...
                    return (
                        <div className="relative flex items-center gap-3 w-[140px]">
                            {/* user.zxFiles[0].id */}
                            <Link
                                href={`/transfer/editor?id=${user.zxFiles[0].id}&order=${user.id}`}
                                underline="none"
                                className="text-sm"
                            >
                                查看结果
                ...
                if (user.zxStatus == 2) {
                    ...
                    return (
                        <div className="relative flex items-center gap-3 w-[140px]">
                            <Link
                                href={`/transfer/settlement?order=${user.id}`}
                                underline="none"
                                className="text-sm"
                            >
                                去支付
            ...
            case "fileName":
                ...
                return (
                    <div>
                        ...
                        >
                            <span className="text-f602">等{names.length}个文件</span>
                        </Tooltip>
                    </div>
                );
